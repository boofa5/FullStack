...
`Summary: Complete Activities 10-15 in Unit 15`
...
* Student's should understand how through the use of validations on their models, they can restrict what type of data can be saved into their databases.
...
* Open the solved file inside `15/11-Sequelize-Validations` and run the program. Show students how they can delete todo items by clicking the `x` button on the todo. Refresh the page to prove that todo is gone.
...
* Slack out the following folder and instructions.
  ...
  * Folder: `15/10/-Sequelize-Update-Delete/10-Sequelize-Update-Delete-Unsolved`.
...
* Slack out the solved folder inside `15/10-Sequelize-Update-Delete` and have students explain to you how the "where" query attribute works when it comes to filtering the records you want to query.
...
* Slack out the following folder in `15/11-Sequelize-Validations`
...
* Slack out `15/11-Sequelize-Validations/11-Sequelize-Validations-Solved` and show students how we added validations and flags to the Todo model. Go through each new item and have students try and explain to you what they believe each piece of new code does.
...
* When time's up, load up the solved version of the activity on your machine `13-Post-Model-Solved`.
...
* Open the [`14-Blog-CRUD-Solved`folder inside of `15/14-Blog-CRUD/2-Blog-CRUD)` and start the server.
...
* Go through the solution in `14-Blog-CRUD-Solved` together. Have students explain to you what each method does, what the "where" attribute is doing in the methods using it.
...
### 15. Instructor Do: Demo Post Author Relationships (15 mins)

* Open the `16-Post-Author-Joins-Solved` application in Chrome and demonstrate how the app's functionality has changed some.
