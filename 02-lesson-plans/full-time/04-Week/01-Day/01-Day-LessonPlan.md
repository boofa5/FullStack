## Day 1 - Intro to Client-Side Storage & Firebase <!--links--> &nbsp; [⬅️](../../03-Week/05-Day/05-Day-LessonPlan.md) &nbsp; [➡️](../02-Day/02-Day-LessonPlan.md)
...
* **Files**:
  ...
  * `01-student-do-todolist` (unsolved)
...
* **Instructions**:
  ...
  * Then create a Basic To-Do application using the `01-student-do-todolist`(unsolved) as a starting point. The HTML is already created for you so you will only have to fill in the javascript sections where there are large blanks. Alternatively, if you are feeling bold and capable, your group may code it from scratch without the use of the base code provided.
...
* Open up the solved file inside `01-todolist-nopersistence` and walk students through the code.
...
* Open solved file inside the `02-signin-nopersistence` folder in the browser and show students the sign-in page. For now the focus should be on creating a sign-in page that simply saves the user input and displays it inside of a text-box.
...
* **Instructions:**
  ...
  * Using the `02-student-do-signin-no-persistence` as a starting point, fill in the JavaScript code necessary to make the page "save user inputs" and then re-display them on the second panel (most recent member).
...
* Open solved file inside the `02-signin-nopersistence` folder and walk students through understanding the code. Make sure they can re-explain the sections to you line by line!
...
* Open the solved file inside `04-signin-localstorage`. in your browser and show them how a re-configured version of the code would work if it utilized localStorage.
...
* Review the code provided in `04-student-do-signin-localstorage-solution` -- paying particular attention to point out all the sections where localStorage is being cleared, set, or obtained (i.e. getItem). Also point out the arguments needed to set and get data.
...
* You can use the solution inside `05-signin-sessionstorage` to quickly show the solution here.
...
* Demonstrate to students a working version of the code in browser using the solution inside `08-todolist-localstorage`.
...
* Walk students through the coded solution inside `08-todolist-localstorage` and use the code comments to help guide the discussion.
...
* Slack out the code inside `instructor-do-clickbutton-fireconsole.html` in `09-clickbutton`.
...
* Review `recentuser-solved.html` in `13-mostrecentuser`. Again focus on the key aspects: the `on.("value")` to track changes, the `.set({})` to add data, etc...
