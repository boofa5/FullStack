...
* **A few days before this class, tell everyone in your class over Slack to download MySQL Workbench so that everyone is using the same program when they get to class!**
	* Students are given instructions in their [Pre-work - Module #5](https://the-coding-bootcamp.gitbooks.io/pre-work-book/content/?key=CodingRocksAJ16) to download MySQL Workbench for Windows, and Sequel Pro for MacOS, as it is more stable. The visual differences are minimal. Sequel Pro users will click 'run selection' whereas MySQL Workbench users will click the lightning bolt. You may ask the class to use only MySQL Workbench so everyone has the same interface, and let MacOS users know they may switch to Sequel Pro if they have stability issues.
...
* Students should be able to explain callbacks and closures.
