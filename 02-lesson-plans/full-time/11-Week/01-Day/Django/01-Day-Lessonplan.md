...
* Finally, point out the `while True` trick, as well as the function of `break` and `continue`.
  ...
  * Let students know they'll need this for thier activity!
...
* Next, demonstrate Python's kewyord arguments.

  * Emphasize that these allow us to define functions whose arguments are passed by _name_, rather than by position.

  * Point out that we can use positional arguments alongside keyword arguments, but that keyword argumenmts _must follow_ positional args.
...
* Finally, ask someone to walk through how they restructured the logic of their main program.
  ...
  * Point out how much more succint and readable this has become, in comparison to the first script we wrote in the beginning of class.
