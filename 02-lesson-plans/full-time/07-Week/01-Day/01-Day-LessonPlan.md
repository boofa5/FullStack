...
* You will be leading them through the beginning of a _heavily_ guided application build that uses an Express server to house and relay data about various Star Wars Characters [starwars-express.herokuapp.com](http://starwars-express.herokuapp.com)
...
* Run `02-Two-Servers/server.js` and demonstrate how the message differs when you visit `http://localhost:7000` vs `http://localhost:7500`.
...
* Open the file `server.js` inside of `03-Portfolio`. Run the application using Node.js.
  ...
  ![07-Portfolio.png](Images/07-Portfolio.png)
...
* Next, run the `server.js` file found in `05-Serve-Favorites`. Visit each of the routes in that file (i.e. `localhost:8080/food`, `localhost:8080/movies`, etc.). Point out how the contents of the page changes each time.
...
* Open `05-Request-Methods/server.js` in your editor.
...
### 14. Student Do: POST Requests (0:20)
...
### 16.	Everyone Do: Introduce Express (0:04)
...
### 17.	Instructor Do: Demo basic routing with server1.js (0:05)
...
### 18.	Students Do: Add Route to server1.js (0:07)
...
### 19.	Instructor Do: Review Previous Activity	(0:05)
...
### 20.	Partners Do: Dissect req.params (0:05)
...
### 21.	Instructor Do: Review Previous Activity	(0:03)
...
### 22.	Partners Do: Dissect Parameter Match (0:05)
...
### 23.	Instructor Do: Review Previous Activity (0:05)
...
Tell students this is a **really** challenging unit, but that it will make more sense after next class. Answer lingering questions.
