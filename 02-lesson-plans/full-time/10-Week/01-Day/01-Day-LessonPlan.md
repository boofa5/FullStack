## Day 1 - An ES6travaganza <!--links--> &nbsp; [⬅️](../../09-Week/05-Day/05-Day-LessonPlan.md) &nbsp; [➡️](../02-Day/02-Day-LessonPlan.md)
...
* We recommend you run the following command to alias `bnode`. It'll allow you to run scripts with Babel cli more easily throughout the class. You can get by installing for every activity and using npm scripts, but it's less smooth.
  ```bash
    npm install -g babel-cli babel-preset-env babel-preset-stage-2 && echo "alias bnode='babel-node --presets=env,stage-2'" >>
     ~/.bash_profile
  ```
...
* Great! So we have block scope down. But syntactically, there's still something annoying in this tacos solution. Ask the students if they can pick it out. (If noone answers up, give a hint: there are a lot of pluses...)
...
* Go through and run the code using the `bnode` alias (see the [Babel Setup Instructions](../../../../01-Class-Content/19-react/01-Activities/110-Stu_BabelSetup/README.md) to set this up if you haven't already).
...
* Explain that they have a breakdown explanation of what this does in their repo, but for now they just need to know it allows us to use `bnode script.js` to run the compiled file instead of `babel-node --presets es2015,stage-2 script.js`.

* Explain to students that they can test out the installation by creating a simple JavaScript file and running `bnode <filename>`. If sucessful, the code should run. For now, we aren't worried about which features are or aren't already supported in the version of Node they're running.

* Walk around and ensure every student gets this setup with the alias correctly before moving on. Have students help each other!
...
* Go through and explain the code inside of the Vehicle class.
  ...
  * Demonstrate how the `constructor` method inside of the class accepts some arguments and assigns them to `this` inside of the constructor method. Point out how we still create object from a class using the `new` keyword, and the values we pass in are equivelent to how values are passed to and used in regular JavaScript constructor functions.

  * Point out how we define methods defined on this class (`accelerate`, `brake`). Point out how the syntax looks halfway between a function call and function definition, and there aren't any commas or colons seperating them.

* Uncomment the code below and demonstrate the ES5 constructor equivelent.

  * Point out how we have to define methods on the prototype seperate from our actual constructor function.
...
### 31. Instructor Do: Recap Objectives and Go over Lingering Questions
